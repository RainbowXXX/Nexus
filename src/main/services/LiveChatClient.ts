...
export default class LiveChatClient {
	 ...
	 */
	async wssSend(data: WSSRequestParameter, timeoutMs: number, exceptReply: boolean): Promise<Result> {
		let result_case = new Promise<Result>(async (resolve, _reject) => {
	...
	async login(loginInfo: parameter.LoginParameter): Promise<Result<string>> {
		...
		return (await this.handleServerEvent('login', Result.Some(token))) as Result;
	}
